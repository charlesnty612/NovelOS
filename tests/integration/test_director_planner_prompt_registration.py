...
1. ``docs/agents/prompts/director_planner-v2.md`` 经 ``PromptRegistry.sync_from_docs``
   注册为 ``('director_planner', 2)``，``get_active_prompt`` 取回内容与文件逐字一致
   （版本标签 ``director_planner:v2``）；
...
注册侧版本标签 = 文件名版本（v2，2026-09-15 大纲槽加固；v1 保留在库内可回溯），二者映射关系由本测试钉住。
"""
...
PROMPT_FILE = PROMPTS_DIR / "director_planner-v2.md"
...
_PROMPT_MARKERS: tuple[str, ...] = (
    ...
    "ratio_declarations",
    "分摊口径（v0.2 加固）",
    # v2 大纲槽（2026-09-15）：大纲优先规则 + 输入契约 chapter.outline
    "大纲优先（最高优先级，先于第 1 条）",
    '"outline": {',
    # 双契约输出结构
    '"scene_plan"',
...
def test_sync_from_docs_registers_director_planner(tmp_path: Path):
    ...
    assert ("director_planner", 2) in result["scanned"]
    assert ("director_planner", 2) in result["registered"]
    ...
    assert version_label == "director_planner:v2"
...
def test_sync_from_docs_is_idempotent_for_director_planner(tmp_path: Path):
    ...
    assert ("director_planner", 2) in first["registered"]

    second = registry.sync_from_docs(PROMPTS_DIR)
    assert ("director_planner", 2) in second["registered"]
    assert ("director_planner", 2) not in second["updated"], "内容未变不应触发 UPDATE"


def test_director_planner_prompt_keeps_v02_hardening_markers():
    text = PROMPT_FILE.read_text(encoding="utf-8")
    missing = [m for m in _PROMPT_MARKERS if m not in text]
    assert not missing, f"director_planner-v2.md 丢失 v0.2 加固短语：{missing}"
