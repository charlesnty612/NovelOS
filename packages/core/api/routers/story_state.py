...
- 404：delta / commit / chapter / project 不存在。
- 409：StateConflictError（状态机非法）、OptimisticLockError（乐观锁）、
       ApprovalRequiredError（HIGH 风险无审批）、FK 冲突（commit 时 chapter/project 不存在）。
...
def get_state(project_id: str, request: Request) -> dict:
    _ensure_project(request, project_id)
    return _service(request).get_current_state(project_id)
...
def submit_delta(project_id: str, payload: dict, request: Request) -> dict:
    _ensure_project(request, project_id)
    result = _service(request).submit_delta(payload)
...
def commit_delta(project_id: str, payload: dict, request: Request) -> dict:
    ...
    try:
        return _service(request).commit_delta(delta_id, author_approval, workflow_run_id)
