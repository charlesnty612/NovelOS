...
本文件保留**全部既有顶层名的再导出**（显式 import 全量保留），
任何 ``from packages.core.context_engine.builders import X``（含私有名）
与 ``builders.X`` 属性访问行为不变；新代码请直接从子模块导入。

``__all__`` 按「导出名必须有消费方或测试引用」收缩（V4.0 结构性负债清理）：
仅保留仓库内确有 ``from ...builders import X`` / ``builders.X`` 消费的名字；
其余名字仍可显式 import，只是不再进 ``import *`` 面与文档面。
"""

from __future__ import annotations

# 门面全量再导出：未进 ``__all__`` 的名字（无消费方）仍保留 import 本体，
# 使 ``from packages.core.context_engine.builders import X`` 深路径 import
# 与 ``builders.X`` 属性访问对全部既有顶层名行为不变（V4.0 清理口径；
# 本文件的 F401 由 pyproject per-file-ignores 承接，不用文件级 noqa 以免掩盖
# 未来真实的未使用 import）。
